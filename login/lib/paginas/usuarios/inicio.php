...
<head>
    ...
    <script>
        $(document).ready(function() {
            function atualizarTabelaVotar() {
                $.ajax({
                    type: 'POST',
                    url: 'lista_votar.php',
                    success: function(response) {
                        $('#tabela-votar').html(response);
                    }
                });
            }

            atualizarTabelaVotar();

            // Atualiza a tabela a cada 30 segundos
            setInterval(atualizarTabelaVotar, 30000);
...
<body>
    ...
    ?>
        ...
        <p>
            Os demais que não estiverem nessa lista não estão aptos para participarem das atividades. Procure o tesoureiro para mais informações!
        </p>            
    <?php
        } else {
    ?>
        <h1>Procure o tesoureiro!</h1>
    <?php
        }
    ?>
